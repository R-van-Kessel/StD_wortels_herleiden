...
const SUBTITLE_TEXT = 'Herleid de wortels en roep de draak op!';
...
function showInfo() {
    ...
    popup.innerHTML = `
        ...
        <p style="color: #0E0E0E; line-height: 1.2;">
            <strong>Doel:<br></strong> Los alle 10 wortelsommen correct op en speel de Dragon game!</ol><br><br>
            ...
            <strong>Wortels herleiden:</strong>
...
function maakVereenvoudigMetCoef() {
    // Genereer c√(a²·k) → ca√k   bijv. 3√50 = 3·5√2 = 15√2
    // c = coëfficiënt voor de wortel (2..5)
    // a = factor onder de wortel (2..6)
    // k = niet-kwadraat (2,3,5,6,7)
    // Limiet: getal onder wortel ≤ 200, antwoord-coëfficiënt ≤ 50
    let nietKwadraten = [2, 3, 5, 6, 7];
    let c, a, k, getal, antwoordCoef;
    let pogingen = 0;
    do {
        c     = floor(random(2, 6));       // coëfficiënt vóór wortel: 2..5
        a     = floor(random(2, 7));       // factor die eruit komt: 2..6
        k     = random(nietKwadraten);
        getal = a * a * k;
        antwoordCoef = c * a;
        pogingen++;
    } while ((getal > 200 || antwoordCoef > 50) && pogingen < 30);
    return {
        text:   c + '√' + getal,
        answer: antwoordCoef + '√' + k
    };
}
...
function maakDelen() {
    // Wortels delen: twee varianten die altijd netjes uitkomen
    // Variant A: (a√k) / (b√k) = a/b   (zelfde wortel, a deelbaar door b)
    //   bijv. 6√3 / 2√3 = 3
    // Variant B: (a√(k·m²)) / (b√k) = (a·m)/b   (vereenvoudigen + delen)
    //   bijv. 6√12 / 2√3 = 6√(4·3) / 2√3 = (6·2√3) / (2√3) = 6
    //   → eenvoudiger: antwoord = (a*m)/b, kies a,b,m zo dat a*m deelbaar is door b
    ...
    if (variant === 'A') {
        // (a√k) / (b√k) = a/b
        // Kies b eerst, dan a als veelvoud van b zodat a/b een geheel getal is
        let k = random(nietKwadraten);
        let b = floor(random(2, 5));          // deler: 2, 3 of 4
        let q = floor(random(2, 6));          // quotient: 2 t/m 5
        let a = b * q;                        // teller = b × quotient
        return {
            text:   a + '√' + k + ' : ' + b + '√' + k,
            answer: '' + q
        };
        ...
        // (a√(k·m²)) / (b√k) = (a·m) / b
        // √(k·m²) / √k = √m² = m, dus: a·m / b — kies zo dat deelbaar
        let k = random(nietKwadraten);
        let m = floor(random(2, 4));          // m = 2 of 3
        let b = floor(random(2, 4));          // deler
        let q = floor(random(2, 5));          // quotient
        let a = (b * q);                      // zorg dat a·m / b = q·m (geheel)
        let teller = k * m * m;               // getal onder teller-wortel
        let antwoord = q * m;
        return {
            text:   a + '√' + teller + ' : ' + b + '√' + k,
            answer: '' + antwoord
...
function generateQuestions() {
    ...
    // Verdeling: 2× vereenvoudigen, 2× vereenvoudigen met coëf, 2× optellen, 1× product, 1× product met coëf, 1× delen, 1× eenvoudig
    for (let i = 0; i < 2; i++) voegToe(maakVereenvoudig);
    for (let i = 0; i < 2; i++) voegToe(maakVereenvoudigMetCoef);
    for (let i = 0; i < 2; i++) voegToe(maakOptellen);
    for (let i = 0; i < 1; i++) voegToe(maakProduct);
    for (let i = 0; i < 1; i++) voegToe(maakProductMetCoef);
    for (let i = 0; i < 1; i++) voegToe(maakDelen);
    for (let i = 0; i < 1; i++) voegToe(maakEenvoudig);
